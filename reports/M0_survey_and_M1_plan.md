...
### 3.3 STEP 3 조사 결과 — 실측 완료 (2026-08-26, 로컬 탐침 실행)

사용자가 로컬 Windows에서 `src/crawler/probe.py`를 실행(총 5요청, 2초 간격)하여
전달한 출력 기준. 전문은 `reports/probe_output.txt`(사용자 로컬) 참조.

| # | 요구 항목 | 실측 결과 |
|---|---|---|
| 1 | robots.txt | **크롤링 허용.** `User-agent: *`에 대해 `/guestbook`, `/m/guestbook`, `/manage`, `/owner`, `/admin`, `/search`, `/m/search`만 Disallow. `/`·`/category/*` 허용. `bingbot`에만 `Crawl-delay: 20` (우리 UA 무관, 자체 2초 지연 유지) |
| 2 | 페이지네이션 | `?page=N` 쿼리 파라미터. `.pagination` 위젯 존재. "실전 투자 전략" 상위 카테고리 최대 16페이지, "투자의 기초" 최대 7페이지 관측 |
| 3 | 글 URL 패턴 | `/entry/<percent-encoded 한글 슬러그>` (다수) + 숫자형 `/<N>` (일부, 구형 글/위젯 링크) |
| 4 | DOM 셀렉터 | 제목: `meta[property='og:title']` (1개 매치, 유일). 작성일: `meta[property='article:published_time']` (ISO 8601 +09:00, 1개 매치, 유일 — `span.date`는 21개 매치로 부적합). 본문: `.tt_article_useless_p_margin` (1개 매치) = `.contents_style` (동일 내용, 폴백). `.entry-content`는 광고 텍스트("728x90 반응형") 혼입으로 **기각** |
| 5 | 이미지 판별 | 가능. 본문 컨테이너 내 `<img>` 스코프 카운트 필수 (샘플: 본문 8개 vs 페이지 전체 34개 — 전체 카운트는 오염됨) |
| 6 | 글 수 대조 | **정확히 일치.** "실전 투자 전략" 표시 172 = 예상 172, "투자의 기초" 표시 181 = 예상 181. 보정 불필요 |

**추가 실측 사실**
- 두 대상 카테고리는 상위 카테고리 "systrader79 칼럼"(451)의 **하위 카테고리**다:
  - `/category/systrader79 칼럼/실전 투자 전략` (172)
  - `/category/systrader79 칼럼/투자의 기초` (181)
  - 상위 451 = 172 + 181 + 1("주식 초보 필수 지식") + 97("ETF 모델 포트폴리오") — 산술 일치
- **목록 페이지 오염**: 페이지당 실제 글 ~28개인데 링크는 54~55개 검출됨.
  사이드바 위젯(최근글/인기글 등)이 원인 → 크롤러는 "여러 페이지에 반복 등장하는
  링크 = 위젯 크롬"으로 필터링하고, 최종 수집 수를 172/181과 대조해 자가 검증한다.
- 샘플 글 작성일 2025-10-14 → 블로그가 최근까지 갱신 중. 수집 시점 기록 필요.
