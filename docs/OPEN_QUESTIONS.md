...
전문은 `reports/M4_inSample_reproduction.md` §5. **전부 blocking (게이트 대기)**.

| # | 상태 | 질문 | 권장안 |
|---|---|---|---|
| Q9 | blocking | M4를 "재현 성공"이 아닌 **"엔진 검증 통과"** 기준으로 종결해도 되는가 | **승인.** 재현은 수단(§0). 격차는 규약·기간·미명시 파라미터로 분해해 한계 등재 |
| Q10 | blocking | C6·C9의 미명시 파라미터를 그리드 서치로 원문 수치에 맞출 것인가 | **하지 않음.** §1-1 위반이자 OOS 판정에 인샘플 과최적화 주입 |
| Q11 | blocking | M5 판정 지표를 **일간 기준**으로 고정할 것인가 | **일간 고정.** 벤치마크와 동일 규약이면 충분, MDD는 일간이 보수적. 원문 대조 표에만 월간 병기 |
| Q12 | blocking | M5에서 C13(Track B, ported)을 함께 판정할 것인가 | **포함.** §3에 따라 Track B는 OOS 판정 대상. `port_note` 병기 |
