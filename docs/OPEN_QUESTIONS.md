...
전문은 `reports/M5_oos_validation.md` §5. **Q13~Q16은 게이트 대기(blocking)**.

| # | 상태 | 질문 | 확정/권장안 |
|---|---|---|---|
| W14 | resolved | M5 로컬 실행 완료(2026-08-31). 17런, 테스트 80/80, `insufficient_sample` 0건 |
| Q13 | blocking | 전 런이 기준 (3)에서 탈락 — 판정 기준 v1.1 조정 여부 | **v1.0 유지 + 위험조정 보조표 병기.** 결과를 본 뒤 기준 변경은 사후 합리화. 변경 시 버전업·전량 재판정·양쪽 병기 |
| Q14 | blocking | 발행일 이후 구간 보조 성과(L-09 대응) 산출 여부 | **M6에서 보조 정보로만 산출.** 판정 구간은 §5대로 2019-01-01 고정 |
| Q15 | blocking | M6 배치 범위 | **C10·C12 추가로 12전략 마감**, C11(숏 필요)·C14·C15(WL 밖 치환)는 정식 제외 확정. 353건 전량 재스크리닝은 미실시 |
| Q16 | blocking | 최종 산출물 형태 | **전략별 1페이지 요약 + 통합 판정표**(한국어), 각 장에 가정값·한계 고정 배치 |
