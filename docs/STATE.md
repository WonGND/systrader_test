...
- M3 계획: ① 엔진+합성 회귀(원격) ② 실데이터 대조(SPY B&H·60/40, 로컬 실행) — 전부 통과 전
  전략 백테스트 금지
- M3 ①단계 완료: 엔진(src/backtest/) + 합성 회귀 8건 전부 통과 (총 15/15).
  회귀 테스트가 엔진 결함 1건(리밸런싱 비용 차감 전 가치 기준 배분 → 미세 음수 현금)을
  실제로 잡아 수정함. ②단계: 사용자 로컬 `python -m tests.regression.run_real_checks`
  실행·출력 전달 대기
