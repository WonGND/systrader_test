...
- M4 1차 실행 완료(2026-08-26 로컬). 결과는 `reports/m4_results.json`
  - **핵심 단서**: 원문(C2) 표의 SPY MDD -50.78%는 SPY의 **월말 기준** 낙폭과 일치.
    일간 기준은 -55.19%(M3 외부 검증). → 원문이 R/월간 데이터로 지표 산출한 정황
  - 2차 실행 준비: 월간 기준 지표, 구간 정렬 SPY 벤치마크, C1 현금군 제외판(원문 9%/-8.52%
    대조용), C9 300일선 진입필터판 추가 (13런)
- M4 구현: Track A 9개 전략 + 오프라인 검증 24건 통과
