...
#
# 저장소 클론/갱신 → 가상환경 → 의존성 → 회귀 테스트 → 백테스트 실행 → 출력 저장
#
# 사용법:
#   .\scripts\run_local.ps1              # M5 OOS 검증 (기본)
#   .\scripts\run_local.ps1 -Task m4     # M4 인샘플 재현
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[!] 회귀 테스트 실패 — 이 출력을 Claude에게 전달하세요. 백테스트는 실행하지 않습니다." -ForegroundColor Red
    exit 1
}
...
if ($Task -eq "m4") {
    $Module  = "src.validate.run_m4"
    $OutFile = Join-Path $Repo "reports\m4_output.txt"
    $JsonRel = "reports\m4_results.json"
    $Label   = "M4 인샘플 재현"
} else {
    $Module  = "src.validate.run_m5"
    $OutFile = Join-Path $Repo "reports\m5_output.txt"
    $JsonRel = "reports\m5_results.json"
    $Label   = "M5 OOS 검증"
}
Write-Host "[5/5] $Label 실행 (첫 실행은 시세 다운로드로 1~3분)" -ForegroundColor Cyan
& $Py -m $Module --json $JsonRel 2>&1 | Tee-Object -FilePath $OutFile

Write-Host "`n출력 저장 위치:" -ForegroundColor Green
Write-Host "  $OutFile"
Write-Host "  $(Join-Path $Repo $JsonRel)"
